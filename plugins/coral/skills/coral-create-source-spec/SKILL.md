...
- OAuth credential methods support device-code flow and authorization-code flow. For authorization-code flow, set `flow.pkce` explicitly to `required` or `disabled`, use a loopback `http://127.0.0.1` or `http://localhost` redirect URI, choose `redirect_uri_port_mode: random` for provider apps that allow variable localhost ports, and choose `fixed` only when users can register the exact non-zero redirect URI. For device-code flow, declare `endpoints.device_authorization_url` and `endpoints.token_url`, omit redirect URI fields, and do not declare a client secret.
