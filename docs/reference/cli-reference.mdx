...
Rebuild materialized artifacts for an installed DSL v4 source without changing
its manifest, variables, or secrets.

```shellscript
coral source refresh github
coral source refresh github --json
```

Refresh re-fetches or re-reads the pinned surface descriptors declared by the
manifest, verifies their SHA-256 hashes, regenerates semantic IR and SQL
projections, and atomically replaces `materialized/v4` only after the new
artifacts validate. It does not run provider data queries or `test_queries`.
...
Enable preview DSL v4 source specs with:
...
Disabling `dsl_v4` leaves already-installed v4 sources in config, but they
cannot be used again until the feature is re-enabled.
