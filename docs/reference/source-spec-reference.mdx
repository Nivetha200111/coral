...
Enable it before installing preview bundled sources or importing a custom
`dsl_version: 4` manifest:

```shellscript
coral features enable dsl_v4
```

Preview bundled v4 sources use distinct schema names such as `github_v4`, so
